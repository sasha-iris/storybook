...
function drawerNavigation(dark) {
  const bg      = dark ? T.bgDark : T.bg;
  const navText = dark ? '#e5e7eb' : T.navText;
  const navIcon = dark ? '#9ca3af' : T.navIcon;
  const active  = dark ? '#374151' : T.navActive;
  const sep     = dark ? '#374151' : T.border;
...
function drawerContactForm(dark) {
  const bg     = dark ? T.bgDark : T.bg;
  const body   = dark ? T.bodyDark : T.body;
  const label  = dark ? '#e5e7eb' : T.label;
  const inputB = dark ? '#4b5563' : T.inputBg;
  const inputC = dark ? '#e5e7eb' : T.inputBorder;
  ...
  <div style="padding:24px 16px;">
    ...
    <div style="display:flex;flex-direction:column;gap:4px;padding-top:8px;border-top:1px solid ${dark ? '#374151' : T.border};">
  ...
  render: (args) => {
    ...
    return `
      <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:40px;align-items:start;">
        <div style="padding:20px;border:1px solid #e5e7eb;border-radius:8px;position:relative;height:520px;overflow:hidden;">
          ${drawer(args)}
        </div>
