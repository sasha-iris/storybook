/**
 * Iris Library — Card icon set
 *
 * Inline SVG strings for use inside `.card-icon` badge containers.
 * All icons use Feather/Heroicons conventions:
 *   - 24×24 viewBox, rendered at 20×20 (width/height attributes)
 *   - stroke="currentColor" — picks up `.card-icon-*` colour automatically
 *   - fill="none", stroke-width="2", round caps/joins
 *
 * Usage in story render functions:
 *   import { ICON } from './card-icons.js';
 *   `<div class="card-icon card-icon-blue">${ICON.revenue}</div>`
 *
 * For compact (32–36 px) icon containers, override the svg dimensions:
 *   ICON.revenue.replace('width="20" height="20"', 'width="16" height="16"')
 */

export const ICON = {

  /** Trending-up arrow — revenue, growth, MRR, positive metrics */
  ...
  /** Two-person silhouette — users, customers, team members */
  users: `<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>`,

  /** Shopping bag — orders, transactions, e-commerce */
  ...
  /** Vertical bar chart — conversion, analytics, counts */
  ...
  /** Trending-down arrow — churn, declining metrics */
  trendDn: `<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="23 18 13.5 8.5 8.5 13.5 1 6"/><polyline points="17 18 23 18 23 12"/></svg>`,

  /** Clock — time-based metrics, session length, latency */
  clock: `<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>`,

  /** Speech bubble — tickets, messages, support volume */
  ...
  /** Eye — page views, impressions, visibility */
  ...
  /** Paint palette — design tokens, theming, colour */
  palette: `<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="13.5" cy="6.5" r=".5" fill="currentColor"/><circle cx="17.5" cy="10.5" r=".5" fill="currentColor"/><circle cx="8.5" cy="7.5" r=".5" fill="currentColor"/><circle cx="6.5" cy="12.5" r=".5" fill="currentColor"/><path d="M12 2C6.5 2 2 6.5 2 12s4.5 10 10 10c.926 0 1.648-.746 1.648-1.688 0-.437-.18-.835-.437-1.125-.29-.289-.438-.652-.438-1.125a1.64 1.64 0 0 1 1.668-1.668h1.996c3.051 0 5.555-2.503 5.555-5.554C21.965 6.012 17.461 2 12 2z"/></svg>`,

  /** 2×2 grid squares — components, modules, library items */
  squares: `<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/></svg>`,

  /** Shield with checkmark — accessibility, security, compliance */
  ...
  /** Alert triangle — errors, warnings, caution states */
  ...
  /** Rotate CCW — refunds, returns, undo actions */
  undo: `<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="1 4 1 10 7 10"/><path d="M3.51 15a9 9 0 1 0 2.13-9.36L1 10"/></svg>`,

  /** Inbox tray — empty states, no data */
  ...
  /** X circle — error states, failed operations */
  ...
  /** Wifi off — no connection, network error */
...
/**
 * Returns an icon SVG resized for compact card-icon containers (32–36px).
 * Replaces the default 20×20 with 16×16.
 * @param {string} svg - An ICON.* value
 * @returns {string}
 */
export const compact = (svg) =>
  svg.replace('width="20" height="20"', 'width="16" height="16"');
