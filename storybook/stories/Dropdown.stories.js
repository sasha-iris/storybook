...
export const OutlineTrigger = {
  name: 'Triggers — outline (filters/selectors)',
  parameters: {
    controls: { disable: true },
    docs: {
      description: {
        story: `
Light filter/selector trigger buttons for dropdowns like "All vendors", "Category", "Location", etc.

**Features:**
- Transparent background with gray border (#e5e7eb)
- Dark text (#1e2939)
- Icon on left + text + ChevronDown on right (auto-aligned via margin-left: auto)
- Gap: 8px between elements
- Hover: light gray background (#f3f4f6)

**CSS:** \`.dropdown-trigger .dropdown-trigger--outline\`

Use with @heroicons/react/24/outline icons. ChevronDownIcon always goes at the end.
        `,
      },
      source: {
        language: 'html',
        code: `<!-- Filter selector with icon and chevron -->
<button class="dropdown-trigger dropdown-trigger--outline">
  <!-- CategoryIcon from heroicons -->
  Category
  <!-- ChevronDownIcon -->
</button>`,
      },
...
<div style="display:flex;flex-direction:column;gap:24px;">
  ...
  <div>
    <p style="font-size:11px;font-weight:600;text-transform:uppercase;color:var(--color-text-secondary);margin-bottom:12px;">Outline selectors (light)</p>
    <div style="display:flex;gap:12px;flex-wrap:wrap;align-items:flex-start;">
      <button class="dropdown-trigger dropdown-trigger--outline">${SEARCH_ICON} All vendors ${CHEVRON_DOWN}</button>
      <button class="dropdown-trigger dropdown-trigger--outline">${COG} Category ${CHEVRON_DOWN}</button>
      <button class="dropdown-trigger dropdown-trigger--outline">${ARCHIVE} Location ${CHEVRON_DOWN}</button>
      <button class="dropdown-trigger dropdown-trigger--outline">${DOCUMENT} Cover ${CHEVRON_DOWN}</button>
    </div>
  </div>

  <div>
    <p style="font-size:11px;font-weight:600;text-transform:uppercase;color:var(--color-text-secondary);margin-bottom:12px;">Hover state</p>
    <div style="display:flex;gap:12px;flex-wrap:wrap;align-items:flex-start;">
      <button class="dropdown-trigger dropdown-trigger--outline" style="background:var(--color-bg-muted);">${SEARCH_ICON} Hovering... ${CHEVRON_DOWN}</button>
    </div>
  </div>
