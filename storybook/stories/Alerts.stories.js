...
function renderAlert({ color = 'success', type = 'medium', heading, body, cta }) {
  ...
  if (type === 'dark') {
    ...
    alertClass = 'alert';
    alertStyle = ` style="background:${tok.darkBg};color:var(--color-bg-white);border-color:${tok.darkBg};max-width:640px;"`;
  } else if (type === 'light') {
    // CSS color class sets text/border; override background to white
    alertClass = `alert alert-${cssColor}`;
    alertStyle = ` style="background:var(--color-bg-white);max-width:640px;"`;
  } else {
  ...
  const btnStyle = type === 'dark'
    ? ` style="background:var(--color-bg-white);color:${tok.accent};border-color:var(--color-bg-white);"`
    : '';
  const ctaHtml = cta ? `
